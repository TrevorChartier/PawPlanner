...
async function createTask(req, res) {
  ...
  if (!(await validatePetID(req.body.pet_id, res))) return; //Supplied ID must correspond to existing Pet ID
...
async function getTasksByPetID(req, res) {
  if (!(await validatePetID(req.params.petID, res))) return; //Supplied ID must correspond to existing Pet ID

  const tasks = await taskModels.getTasksByPet(req.params.petID);

  if (!tasks) return res.status(404).send('Tasks with specified petID not found');
